...
PAGES = [
    ...
    {"title": "Streaming Variational Family", "paragraphs": [
        ...
        "An initialization-only symmetry break is permitted because identical component factors otherwise yield identical responsibilities. Bootstrap centers are used only to initialize the first responsibility calculation; they are not retained as pseudo-observations in the posterior sufficient statistics. Each arriving batch then receives one deterministic responsibility-and-sufficient-statistic update. This is a one-pass streaming approximation, not an inner coordinate-ascent loop.",
    ]},
    ...
    {"title": "Primary and Secondary Endpoints", "paragraphs": [
        ...
        "Compute endpoints include synchronized model latency per retained observation. Network time is excluded from model latency. Structural endpoints include occupied-cluster count, posterior residual-stick mass for VI, and mean particle ESS for SMC. Diagnostic failure is a hard-stop condition rather than a silently filtered outcome.",
    ...
    {"title": "Paired Block-Contrast Model", "paragraphs": [
        "For each endpoint, the analyzer first forms within-block SMC-minus-VI differences at every thinning level. It then reports the three cell-specific architecture contrasts plus architecture-average, linear-thinning, quadratic-thinning, architecture-by-linear, and architecture-by-quadratic contrasts. The recorded block is the unit of independent replication.",
        "Latency is analyzed on the log scale because it is strictly positive and commonly right-skewed. Predictive log score, occupied-cluster count, and evaluator-only category agreement remain on their observed scales. Each contrast uses a paired t interval across blocks; missing category agreement is dropped only for that secondary contrast and its valid block count is reported.",
        "The architecture average summarizes separation over thinning levels. The architecture-by-linear contrast asks whether method separation changes monotonically as observations are removed; the quadratic interaction asks whether that change curves. Two-sided p-values receive an endpoint-wise Holm adjustment across the eight prespecified contrasts, while estimates and uncertainty intervals remain primary.",
    ], "equation": "D_bd = Y_SMC,bd - Y_VI,bd; contrasts are estimated across independent recorded blocks b."},
    ...
    {"title": "Convergence and Monte Carlo Diagnostics", "paragraphs": [
        ...
        "For SMC, particle ESS is tracked during updates and the cell record retains its mean. Equal particle weights after resampling do not by themselves prove partition diversity, so the public result makes no broader convergence claim. For VI, the cell record retains the residual-stick weight and occupied-component count; nonfinite primary outcomes fail the completeness gate.",
    ...
    {"title": "Latency and Streaming-Systems Measurement", "paragraphs": [
        ...
        "A deterministic warm-up execution occurs before the timed factorial run. Execution order is randomized within block. The environment manifest records processor, accelerator, operating system, Python and package versions, precision, particle count, truncation, and device. Latency is analyzed on the log scale with the recorded block as the unit of independent replication.",
    ...
    {"title": "Reproducibility and Data Availability", "paragraphs": [
        ...
        "The public artifact is designed for audit. Every dashboard number is either a structural design quantity, a software version fact, or clearly labeled synthetic illustration. The empirical status flag remains false until the complete verified factorial passes the analysis gate. No inferential estimate is backfilled with simulated or invented numbers.",
    ...
    {"title": "Appendix A: Algorithmic Pseudocode", "paragraphs": [
        ...
        "Frozen experiment: record each eligible block once; checksum and structurally validate it. Construct nested deterministic masks. Randomize six cell orders. Execute the two inference methods under three masks with the verified lock. Freeze outputs. Join evaluator-only labels. Compute paired block-level contrasts, endpoint-wise Holm adjustments, and provenance hashes before publishing aggregate diagnostics.",
    ]},
